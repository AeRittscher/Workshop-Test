...
suppressWarnings(require(cowplot)) # suppress the new theme command change
theme_set(theme_cowplot()) # since version 1.0 explicit setting required
require(phyloseq)
require(dada2)
```
...
#Load in Data
```{r}
Pilot16S <- readRDS("20241018_AR_16S_silva_merged_NAfilled.rds")
```
#Phyloseq basics
```{r}
...
# tax_table(Pilot16S)[1:5, 1:4]

# phy_tree(Pilot16S)
# myTaxa = names(sort(taxa_sums(Pilot16S), decreasing = TRUE)[1:10])
# ex1 = prune_taxa(myTaxa, Pilot16S)
...
# renaming samples
# load metadata file
metaPilot <- read_excel("NCOH-PP Pilot Sample Master List.xlsx", sheet = "Sample Master List")
...
```{r}
# Load 16S Data
qPCR16SPilot <- read_excel("NCOH-PP Pilot Sample Master List.xlsx", sheet = "16S qPCR")
...
###Air
```{r}
# Split_by_Sample_Type<-function(dataframe,col_name) {
# air_dataset<- subset_samples(dataframe,col_name == ("Air Filter"))
# water_dataset<-
#  subset_samples(dataframe, col_name == "Water")
#  return(c(air_dataset,water_dataset))}

# Split_by_Sample_Type(Pilot16S,col_name = Sample_Type)
...
# |#Air_composition <- create_ordered_bar(subset_samples(Pilot16S, !is.na(population)), "Air Filter", 15) +
...
# labs(title="Top 20 Phyla - Air Samples",fill="Phyla")+
# theme(
#   legend.text = element_text(size = 5),  # Adjusts text size in the legend
#  legend.key.size = unit(0.25, "cm") ) # Adjusts the size of the legend key boxes
```
...
```{r}
#
# merged.r <- transform_sample_counts( merged, function(x) x / sum(x) * 100 )
# bar_genus.r.t25 <- ps_plot_bar(merged.r, top=25, taxrank="Genus")
# SaveVariousPlots( paste0(outfiles,"/",pjbase,"_BarPlots_genus_relative_top25"), bar_genus.r.t25, filetype=filetype)
# Genus
...
# prevalence.plot <- ggplot(prevdf[!(as.numeric(prevdf$Phylum) %in% which(table(prevdf$Phylum)<5)),],
#                    aes(TotalAbundance, Prevalence, color = Phylum)) +
#                   geom_point(show.legend=F) +
#                  scale_x_log10() + xlab("Total Abundance") + ylab("Phylum prevalence") +
#                 facet_wrap(~Phylum) +
...
#PCOA-water
```{r}
ps.r.ord <- ordinate(prevdf_water, method = "PCoA", dist = "bray")
ps.r.ord.p1 <- plot_ordination(prevdf_water, ps.r.ord, shape = "Sublocation", label = "Sample_Number", color = "Location")
ps.r.ord.p1
# ord_plot(color = "Location", shape = "Distance_From_Stable (m)", size = 2) +
# scale_colour_brewer(palette = "Dark2")
```
